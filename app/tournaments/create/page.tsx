...
import { ChevronLeft, ImagePlus, AlertCircle } from "lucide-react";
        ...
        <div className="animate-fade-in-up">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-3">
              <button onClick={() => router.push("/dashboard")} className="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg bg-surface-alt border border-border text-text-muted hover:text-primary hover:border-primary/20 hover:bg-primary-light/30 transition-all cursor-pointer">
                <ChevronLeft className="w-4 h-4" strokeWidth={2.5} />
              </button>
              <h2 className="text-2xl font-bold tracking-tight">Create Tournament</h2>
            </div>
            <p className="text-text-muted text-sm mt-1.5 ml-11">Set up a new cricket auction tournament</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {logoPreview ? (
                <div className="flex items-center gap-4 p-3 rounded-xl bg-surface-alt border border-border">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-text-subtle">{logoFile && (logoFile.size / 1024).toFixed(0)} KB</p>
                  </div>
                  <button type="button" onClick={handleRemoveLogo} className="btn-ghost text-xs text-danger hover:text-danger">
                    Remove
                  </button>
                </div>
              ) : (
            ...
            <div className="flex items-center gap-3 pt-2">
              <button type="submit" disabled={isSubmitting} className="btn-primary !w-auto flex-1 sm:flex-none sm:min-w-[200px]" id="create-tournament-button">
                {isSubmitting ? <><div className="spinner" /><span>Creating...</span></> : "Create Tournament"}
              </button>
